    ...
    "steps": [
        {
            ...
            "state": {"globals": {"transacciones": "[{'id': 'A', ...}, ...]"}}
        },
        {
            "highlight": {"line": 10},
            ...
            "state": {"io": {"log": ["sorted() está usando la lambda para mirar la 'cuantia' de cada elemento."]}}
        },
        {
            "highlight": {"line": 11},
            "what": "El argumento `reverse=True` indica que la ordenación debe ser de mayor a menor.",
            "why": "Las funciones como `sorted` a menudo tienen parámetros adicionales para afinar su comportamiento. Esto las hace aún más flexibles.",
            "appData": "Permite cambiar fácilmente entre orden ascendente y descendente, algo crucial al generar rankings o clasificaciones.",
            "appLaw": "Es un modificador del criterio. El criterio principal es la cuantía, pero este parámetro especifica la dirección de la prelación (en este caso, de la más alta a la más baja, como en una subasta o en la prelación de créditos de mayor cuantía).",
            "state": {}
        },
        {
            "highlight": {"line": 15},
            "what": "El resultado es una nueva lista de transacciones, reordenada según el criterio de la cuantía.",
            ...
            "state": {"globals": {"transacciones_ordenadas": "[{'id': 'C', 'cuantia': 800.0, ...}, ...]"}}
        }
    ]
}
